...
void GlMesh::compileGL() {
    ...
    glAssert( glBufferData(GL_ARRAY_BUFFER,nbVertices_ * sizeof(Vertex), &(vertices_[0]),GL_STATIC_DRAW));
    ...
    glAssert( glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, vertexBufferObjects_[VBO_INDICES]) );
    // 8 - Remplir le VertexBufferObject contenant les faces
    glAssert( glBufferData(GL_ELEMENT_ARRAY_BUFFER, 3*nbTriangles_ * sizeof(int), &(triangles_[0]),GL_STATIC_DRAW) );
