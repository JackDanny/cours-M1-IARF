...
;; it clears the screen and resets the clock
to setup
  ca
end
...
;; its size and if it is on the edge of the screen, dies.
to go
  cro 1
  [
    set color 10 * (ticks mod num-colors) + 15
    set size 0.6 * sqrt distancexyz 0 0 0
    if flower-type = "2D Horizontal"
      [ rt ticks * turn-increment * 360 ]
    if flower-type = "2D Vertical"
      [ tilt-up ticks * turn-increment * 360 ]
    if flower-type = "Pine Cone"
      [ rt ticks * turn-increment * 360
        tilt-up ticks * turn-increment ]
    if flower-type = "Spiral"
      [ rt ticks * turn-increment * 360
        tilt-up ticks * turn-increment * 360 ]
    if roll?
      [ roll-right ticks * turn-increment * 360 ]
  ...
  [
    fd step-size
    set size 0.6 * sqrt distancexyz 0 0 0
    if distancexyz 0 0 0 > (max-pxcor - 0.5)
      [ die ]
  ]
  tick
end
...
flower-type
flower-type
"2D Horizontal" "2D Vertical" "Pine Cone" "Spiral"
2
