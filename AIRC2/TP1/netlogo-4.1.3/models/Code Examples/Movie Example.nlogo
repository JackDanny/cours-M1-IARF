...
This model includes a sample BehaviorSpace "experiment setup" which makes a movie, just like MAKE-MOVIE procedure.  To try it, choose BehaviorSpace on the Tools menu.  In the dialog that opens you'll see an experiment called "make movies".  Select that experiment and hit the "Run" button.  Background runs in parallel BehaviorSpace experiment can't make movies, so change the "Simultaneous number of runs" setting to 1, disabling background runs, and hit "OK".  The model will run three times and a movie files will be generated each time. The BEHAVIORSPACE-RUN-NUMBER primitive is used to give each movie a different name.
