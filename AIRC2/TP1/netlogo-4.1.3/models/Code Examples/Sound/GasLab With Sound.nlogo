...
to setup-plots
  set-current-plot "Number vs. Time"
...
to do-plotting
  ...
  if length pressure-history > 0
    [
       ifelse avg-plot-pressure?  ;; pressure plot is 'smoothed' by averaging 3 sequential values, only if switch is set this way
...
This model simulates pumping up a bicycle tire.  It consists of gas particles that are trapped in a fixed and rigid container.  The model specifically looks at how the number of particles affects the pressure of system.  Air can be added into the container through a valve.  Adding a group of particles into the container increases the number of particles that are moving and colliding with each other and hitting the walls.

This model includes sounds.  Different events and properties can be listened to. You can listen to some of the sounds or all of them, to the group events or to the individual particle events.
In this activity, you may explore the model to find out how the different events in the model are related, and how they change when you add more and more particles:
* Model time and real time
* Macroscopic variables: pressure, wall hits, collisions
* Microscopic variables: particle speed, particle wall hits, particle collisions

This model is part of the Connected Chemistry High School Curriculum and is part of the second and third activities, in which the concept of pressure is explored.  In this model, students look at the relationship between number of particles and pressure. In addition, one can follow the average number of wall hits in one model clock tick.  A clock hand is used to mark the passage of time in the model.  When the particles hit the walls they change their color temporarily.  These models have been adapted from the model GasLab Pressure Box, which can be found in the Unverified section of the GasLab folder found under Chemistry and Physics.
...
HOW IT WORKS
------------
Particles are modeled as perfectly elastic particles with no energy except their kinetic energy -- which is due to their motion.  Collisions between particles are elastic.  Particles are colored according to speed -- blue for slow, green for medium, and red for high speeds.
...
Sounds
The loudness of the announcement on particles entering the box is connected to their number
The pitch on pressure is related to the value of pressure.
Wall hits' pitches are associated with the headings of the hitting particles.
Collisions' pitches are calculated using the sum of the colliding particles' speeds.
The pitch for a single particle's speed is related to its speed.


HOW TO USE IT
-------------
Buttons:
SETUP - sets up the initial conditions set on the sliders.  Be sure to wait till the Setup button stops before pushing GO/STOP.
...
ALL ON - turns on all the sounds
ALL OFF - turns off all the sounds
...
REAL-TIME-PACER - sets the cycle for the real time sound
PRESSURE-LOUDNESS - sets the loudness for pressure's sound
WALL-HITS-LOUDNESS - sets the loudness for wall hits' sound
COLLISIONS-LOUDNESS - sets the loudness for collisions' sound
SINGLE-PARTICLE-LOUDNESS

Switches
The following switches allow adding sounds to the model for the different events
ANNOUNCE-ADD-PARTICLES?
MODEL-TIME?
GROUP-ON?
PRESSURE?
...
SINGLE-SPEED - speed of one particle (ID 3), which is being tracked by sound and   pen-down

Plots:
PRESSURE VS TIME - plots the pressure in the box over time.
NUMBER VS TIME - plots the number of particles in the box over time.  This plot is
laid under the WALL HITS PER PARTICLE plot, as in scripted activities the two plots
can be exposed in different screens.
WALL HITS PER PARTICLE - the average number of wall hits in one clock tick
...
MODEL-TIME: As there are more particles in the model, the model slows down.  Add particles to hear that happen.

Compare MODEL-TIME with REAL-TIME.

Listen to one particle alone (its ID is 3), its speed, collisions and wall hits.  Change the number of particles.  When does the particle change speed more frequently?  When does the particle collide more frequently?

You can listen to some features that involve the whole group: PRESSURE, WALL HITS and COLLISIONS.  Add particles a number of times and notice what happens to these features.

Listen to PRESSURE: can you hear the sounds go up and down the scale together with the Pressure vs. Time plot or the pressure monitor?  Can you hear the plot with WALL HITS?

Turn on the ANNOUNCE-ADD-PARTICLES?  Do you notice the delay in time between injecting particles and the pressure going up?  Listen to the wall-hits as the particles hit the wall.
