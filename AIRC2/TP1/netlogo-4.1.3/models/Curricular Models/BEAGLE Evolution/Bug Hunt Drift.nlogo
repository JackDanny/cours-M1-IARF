...
to go
  ...
  every 0.03 [
    ...
    ;; advance the clock
    tick    ;; plotting takes time, so only plot every 10 ticks
    update-plots
  ]
...
to update-plots
  ...
  foreach [1 2 3 4 5 6] [
    ...
    plotxy ticks count bugs with [color-variation = ?]
  ]


  ;; the HISTOGRAM primitive can't make a multi-colored histogram,
  ;; so instead we plot each bar individually
  set-current-plot "Current Bug Population"
  clear-plot
  foreach [1 2 3 4 5 6] [
    set-current-plot-pen word "" ?
    plotxy ? count bugs with [color-variation = ?]
  ]

  set-current-plot "Bugs Removed"
  clear-plot

  set-histogram-num-bars 8
  set-current-plot-pen "1"
  plotxy 1 total-type-1-removed

  set-current-plot-pen "2"
  plotxy 2 total-type-2-removed

  set-current-plot-pen "3"
  plotxy 3 total-type-3-removed

  set-current-plot-pen "4"
  plotxy 4 total-type-3-removed

  set-current-plot-pen "5"
  plotxy 5  total-type-5-removed

  set-current-plot-pen "6"
  plotxy 6  total-type-6-removed
...
true
PENS
"1" 1.0 0 -8630108 true
"2" 1.0 0 -13345367 true
"3" 1.0 0 -10899396 true
"4" 1.0 0 -6459832 true
"5" 1.0 0 -955883 true
"6" 1.0 0 -2674135 true
...
- Copyright 2009 Uri Wilensky. All rights reserved. See http://ccl.northwestern.edu/netlogo/models/BugHuntDrift for terms of use.
