...
to setup-plot
  set-current-plot "Turtle Position"
  set-plot-y-range  min-pycor max-pycor
end

to update-plot
  set-current-plot-pen "Xcor"
  plot [xcor] of turtle 0         ; plot xcor -- there's only one turtle, so its id must be 0
  set-current-plot-pen "Ycor"
  plot [ycor] of turtle 0         ; plot ycor -- ditto
  if ticks > 800                  ; don't change the range until we've plotted all
                                  ; the way across once
  [
    set-plot-x-range (ticks - 800) ticks  ; scroll the range of the plot so
...
true
PENS
"Xcor" 1.0 0 -2674135 true
"Ycor" 1.0 0 -13345367 true
...
HOW IT WORKS
------------
In the UPDATE-PLOT procedure, the SET-PLOT-X-RANGE command is used to set the visible range to only the most recent 800 ticks.
