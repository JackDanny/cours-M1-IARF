to setup
  ...
  [
    ;; Initially, my-nodes have no connecting edges.  So we set
    ;; out-edges and in-edges to the empty list.
    set shape "circle"
    set color blue
    set size 3
...
;; Here we create and delete edges totally at random, except that
;; we never go below 10 edges.
to go
  ask one-of turtles
    [ form-random-edge ]
  if count links > number-of-edges
    [ ask one-of links
        [ die ] ]
  tick
end
...
to wiggle-nodes
  ask turtles [ fd 0.1 left random 90 tilt-up random 90 ]
  display
end

to form-random-edge ;; node procedure
  ;; pick a random other node
  let other-node one-of other turtles
  ;; make sure we got another node, and that we aren't connected
  ;; to it, and that it isn't connected to us
  if (other-node != nobody) and link-with other-node = nobody
  [
     create-link-with other-node [ set color green ]
  ]
end
