...
The soil is modeled as a checkerboard of hard particles (gray squares) and semi-permeable spaces in between these hard particles (brown squares).
(You may need to zoom in to see the individual squares.)
Oil cannot enter the solid gray squares, but it may pass through the brown squares.
Some soils are more porous ("holey") than other soils.
In this model the porosity value of the soil determines the probability that the oil will be able to enter any given brown soil square.
...
- Copyright 1998 Uri Wilensky. All rights reserved. See http://ccl.northwestern.edu/netlogo/models/Percolation for terms of use.
