...
to back-propagate
  ...
  ask output-node-1 [
    set error activation * (1 - activation) * (answer - activation)
    set example-error example-error + ( (answer - activation) ^ 2 )
  ]
  set epoch-error epoch-error + example-error
  ask hidden-nodes [
    set error activation * (1 - activation) * sum [weight * [error] of end2] of my-out-links
  ]
  ask links [
