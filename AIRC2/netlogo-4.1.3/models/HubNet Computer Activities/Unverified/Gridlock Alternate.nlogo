...
to add-new-client-metric-value  ;; intersection procedure
  let error __check-syntax (word "plotxy clock " my-plot-metric)
  ifelse error = ""
  [
...
to new-client-metric [ current-id new-metric ]
  ask intersections with [ user-id = current-id ]
  [
    let error __check-syntax (word "plotxy ticks " new-metric)
    ;; don't send an error message if the client sent the empty string
    if new-metric != ""
    [
      hubnet-send user-id "metric-error-message" error
      hubnet-send user-id "Current Metric Value" ""
    ]
    ifelse error = ""
    [
...
- Copyright 2002 Uri Wilensky and Walter Stroup. All rights reserved. See http://ccl.northwestern.edu/netlogo/models/HubNetGridlockAlternate for terms of use.
